...
const (
	...
	veleroPackage           = "github.com/velann21/velero/"
...
func (h *LogLocationHook) Fire(entry *logrus.Entry) error {
	...
	// skip 2 frames:
	//   runtime.Callers
	//   github.com/velann21/velero/pkg/util/logging/(*LogLocationHook).Fire
...
func removeVeleroPackagePrefix(file string) string {
	if index := strings.Index(file, veleroPackage); index != -1 {
		// strip off .../github.com/velann21/velero/ so we just have pkg/...
